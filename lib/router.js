...
Router.route('/user/update/:_id',{
  name : 'user.update',
  layoutTemplate : 'layoutAfterAuth',
  waitOn: function() {
    return Meteor.subscribe('singleUserPublish', this.params._id);
  },
  data : function (){
    var getUser = Meteor.users.findOne(this.params._id);
    console.log(Meteor.users.find({_id : this.params._id}));
    console.log(getUser.profile.fullName);
    return {
      createdUser : {
